...
export const veterinaryValidator = [
	...
	check("phone").exists().notEmpty().isNumeric().isLength({ min: 6, max: 10 }),
	// .custom(isValidVeterinaryphone),

	check("email").exists().notEmpty().isLength({ min: 6, max: 50 }).isEmail(),
	// .custom(isValidUser),

	check("description").exists().notEmpty().isLength({ min: 10, max: 200 }),

	check("address").exists().notEmpty().isLength({ min: 5, max: 40 }),
